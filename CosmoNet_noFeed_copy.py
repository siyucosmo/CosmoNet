...
def BatchNorm(inputT, IS_TRAINING, name):
    return tf.cond(tf.constant(IS_TRAINING) > tf.constant(0), 
	lambda: tf.layers.batch_normalization(inputT, training=True, name="conv1_bn"), 
	lambda: tf.layers.batch_normalization(inputT, training=False, name="conv1_bn",reuse = True))
...
class CosmoNet:
    ...
    def deepNet(self,inputBatch,IS_TRAINING,keep_prob):
		...
		name = "conv1_bn_val"
            print name
            #h_conv1 = lrelu(tf.layers.batch_normalization(tf.nn.conv3d(inputBatch, self.W['W_conv1'],strides = [1,1,1,1,1],padding = 'VALID') + self.b['b_conv1'],training = IS_TRAINING, name = name),hp.Model['LEAK_PARAMETER'])
            h_conv1 = lrelu(BatchNorm(tf.nn.conv3d(inputBatch, self.W['W_conv1'],strides = [1,1,1,1,1],padding = 'VALID') + self.b['b_conv1'],IS_TRAINING, name = name),hp.Model['LEAK_PARAMETER'])
    ...
    def loss(self):
        with tf.name_scope('loss'):
            predictions = self.deepNet(inputBatch = self.train_data,IS_TRAINING = 1,keep_prob = hp.Model['DROP_OUT'])
    ...
    def validation_loss(self):
        val_predict = self.deepNet(inputBatch = self.val_data,IS_TRAINING = 0,keep_prob = 1)
	...
	lossL1Val,val_true,val_predict = self.validation_loss()    
        return train_step, loss, lossL1Val,val_true,val_predict
    
    def train(self):
        train_step, loss, lossL1Val,val_true,val_predict = self.optimize()
	...
	config = tf.ConfigProto()
		...
		start_time = time.time()
			...
			if(i%50==0):
				print i 
			_,lossTrain = sess.run([train_step,loss])
                        #loss_per_epoch_train += loss_
                        #lossTrain_per_epoch +=lossTrain
			x = [v.eval() for v in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="conv1_bn")]
                        print x[1]
		...
		for i in range(hp.RUNPARAM['batch_per_epoch_val']):
			loss_,val_true_,val_predict_ = sess.run([lossL1Val,val_true,val_predict])
                        ...
                        x = [v.eval() for v in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope="conv1_bn_val")]
                        print x[1]
		...
		print "  validation loss: %.3f" %(loss_per_epoch_val/hp.RUNPARAM['batch_per_epoch_val'])
                np.savetxt('loss_train_batch450.txt',losses_train)
                np.savetxt('loss_val_batch450.txt',losses_val)
                np.savetxt('losses.txt',losses)
                np.savetxt('/zfsauton/home/siyuh/pred/train_pred'+str(epoch)+'.txt',np.c_[train_true_,train_predict_])
                np.savetxt('/zfsauton/home/siyuh/pred/val_pred'+str(epoch)+'.txt',np.c_[val_true_,val_predict_])
